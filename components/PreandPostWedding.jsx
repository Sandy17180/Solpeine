...
const preandpostImages = [
  {
    id: 1,
    image: "/LifestyleGenre.jpg",
    orientation: "landscape",
  },
  {
    id: 2,
    image: "/pre_and_post_weddingGenre.jpg",
...
export default function PreandPostWedding() {
  const [currentIndex, setCurrentIndex] = useState(0);

  const nextImage = () => {
    setCurrentIndex((prev) => (prev + 1) % preandpostImages.length);
  };

  const prevImage = () => {
    setCurrentIndex((prev) => (prev === 0 ? preandpostImages.length - 1 : prev - 1));
  };
  ...
  const currentImage = preandpostImages[currentIndex];
  ...
  const getMainImageSize=(orientation)=>{
    return orientation === "portrait"
      ? { width: 488, height: 610 }
      : { width: 610, height: 488 };
  };
  ...
  return (
    <div className="flex min-h-screen bg-#F5F1EAE0 text-black px-10 py-8">
      ...
      {/* Thumbnail list */}
<div className="w-[398px] h-[1022px] bg-[#F1D5CAA8] flex flex-col gap-4 px-6 pt-4 overflow-y-auto scrollbar-hide relative z-20 -mt-20 items-center">
        {preandpostImages.map((img, idx) => {
