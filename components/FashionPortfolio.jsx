...
export default function FashionPortfolio() {
  ...
  const nextImage = () => {
    setCurrentIndex((prev) => (prev + 1) % fashionImages.length);
  };

  const prevImage = () => {
    setCurrentIndex((prev) => (prev === 0 ? fashionImages.length - 1 : prev - 1));
  };
  ...
  const currentImage = fashionImages[currentIndex];
  ...
  return (
    <div className="flex min-h-screen bg-#F5F1EAE0 text-black px-10 py-8">
      ...
      {/* Thumbnail list */}
      <div className="w-[398px] h-[1022px] bg-[#F1D5CAA8] flex flex-col gap-4 pl-10 pt-4 overflow-y-auto overflow-x-hidden scrollbar-hide relative z-20 -mt-20 items-center">
